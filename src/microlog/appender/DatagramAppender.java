 ...
 * logging sent to the host at the time of logging (no buffer). The class uses a
 * <code>Datagram</code> that is re-used and filled with new data each time a
 * message is sent.
 * <p>
 * The appender can be configured with {@value #HOST_PROPERTY_STRING} for the
 * host and {@value #PORT_PROPERTY_STRING} for the port.
 * 
 * 
...
public class DatagramAppender extends AbstractAppender {
	 ...
	 * The property to set for the host to connect to.
	 */
	public static final String PORT_PROPERTY_STRING = "microlog.appender.DatagramAppender.port";
	 ...
	 * <code>microlog.appender.DatagramAppender.host</code>. The port address
	 * can be passed with the property
	 * <code>microlog.appender.DatagramAppender.port</code>.
	 * 
	 * @param properties
	...
	public void configure(PropertiesGetter properties) {
		...
		// Set the port address from Properties
		try {
			port = Integer.parseInt(properties.getString(PORT_PROPERTY_STRING));
