...
import microlog.format.command.CategoryFormatCommand;
import microlog.format.command.DateFormatCommand;
import microlog.format.command.FormatCommandInterface;
import microlog.format.command.MessageFormatCommand;
import microlog.format.command.NoFormatCommand;
import microlog.format.command.PriorityFormatCommand;
import microlog.format.command.ThreadFormatCommand;
import microlog.format.command.ThrowableFormatCommand;
import microlog.format.command.TimeFormatCommand;
...
public class PatternFormatter implements Formatter {
	...
	public void configure(PropertiesGetter properties)
		...
		if (properties == null) {
			throw new IllegalArgumentException(
					"The properties argument must not be null");
		}
			...
			if (currentChar == '%') {
				...
				case DATE_CONVERSION_CHAR:
					...
					if (specifierLength > 0) {
						...
						currentIndex = currentIndex + specifierLength;
					}
	...
	String extraxtSpecifier(String pattern, int index) {
		...
		if (beginIndex > 0 && endIndex > beginIndex) {
			specifier = pattern.substring(beginIndex, endIndex);
			System.out.println("Specifier is: " + specifier);
		}
