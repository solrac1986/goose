...
import microlog.time.StopWatch;
import microlog.util.PropertiesGetter;

/**
 * The <code>Logger</code> class is used for logging.
 * 
 * This is similiar to the Log4j <code>Logger</code> class. The method names
 * are the same, but we do not claim that they are 100% compatible.
 * 
 * You have the ability to use named loggers as in Log4j, although Microlog does
 * not put the loggers in a hierarchy. If you want to save memory, you could use
 * unamed loggers.
 * 
...
public final class Logger {
	...
	/**
	 * Get the Logger instance (without the name).
	 * 
	 * @return the Logger.
	 */
	...
	/**
	 * Get the effective <code>Level</code>.
	 */
	public Level getEffectiveLevel() {
		Level effectiveLevel = Level.DEBUG;

		if (logLevel != null) {
			effectiveLevel = logLevel;
		} else {
			effectiveLevel = globalLogLevel;
		}
	...
	public void log(Level level, Object message, Throwable t)
		...
		if (effectiveLevel.levelValue <= level.levelValue) {
			int nofAppenders = appenderList.size();
			...
			for (int index = 0; index < nofAppenders; index++) {
				Appender appender = (Appender) appenderList.elementAt(index);
				appender.doLog(name, stopWatch.getCurrentTime(), level,
						message, t);
			}
		}
	}
	...
	public boolean isTraceEnabled() {
		return getEffectiveLevel().levelValue <= Level.TRACE_INT;
	}
	...
	public boolean isDebugEnabled() {
		return getEffectiveLevel().levelValue <= Level.DEBUG_INT;
	}
	...
	public boolean isInfoEnabled() {
		return getEffectiveLevel().levelValue <= Level.INFO_INT;
	}
	...
	public boolean isWarnEnabled() {
		return getEffectiveLevel().levelValue <= Level.WARN_INT;
	}
	...
	public boolean isErrorEnabled() {
		return getEffectiveLevel().levelValue <= Level.ERROR_INT;
	}
	...
	public boolean isFatalEnabled() {
		return getEffectiveLevel().levelValue <= Level.FATAL_INT;
	}
	...
	public void configure(PropertiesGetter properties)
		...
		Logger.configureLogLevel(properties.getString(LOG_LEVEL_STRING));
		configureAppender(properties);
		configureFormatter(properties);

		// Configure the log level for the different Loggers
		Logger.configureLoggerIndividualLevels(properties);
	}

	/**
	 * Configure the log level for each indivdual <code>Logger</code>
	 * instance.
	 * 
	 * @param properties
	 *            the properties to get the properties from.
	 */
	static void configureLoggerIndividualLevels(PropertiesGetter properties) {
		Enumeration loggerEnum = loggerHashtable.elements();

		while (loggerEnum.hasMoreElements()) {
			Logger logger = (Logger) loggerEnum.nextElement();
			String loggerName = logger.getName();
			if (loggerName != null && loggerName.length() > 0) {
				StringBuffer logLevelStringBuffer = new StringBuffer(128);
				logLevelStringBuffer.append(LOG_LEVEL_STRING);
				logLevelStringBuffer.append('.');
				logLevelStringBuffer.append(logger.getName());
				configureLoggerLogLevel(logger, properties
						.getString(logLevelStringBuffer.toString()));
			}
		}
	}
	 ...
	 *            The logLevel to set.
	 */
	static void configureLoggerLogLevel(Logger logger, String logLevel) {
		...
		if (logLevel != null) {
			...
			System.out.println("Log level for " + logger.getName()
					+ " is configured to " + logger.getLogLevel());
		}
	}
	 ...
	 * @param logLevel
	 */
	static void configureLogLevel(String logLevel) {
		if (logLevel != null) {
			if (logLevel.compareTo(Level.FATAL_STRING) == 0) {
				...
				Logger.setGlobalLogLevel(Level.WARN);
			} else if (logLevel.compareTo(Level.INFO_STRING) == 0) {
			...
			System.out.println("Global log level is configured to "
